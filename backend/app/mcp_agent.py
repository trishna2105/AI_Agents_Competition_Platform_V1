...
def stream_agent(competition_id: int, agent_id: int, prompt: str, reasoning: str = "") -> str:
    ...
    Will automatically wait for the competition to become 'ongoing' before submitting.
    """
    # Hard gate: poll until ongoing, cannot be skipped
    timeout = 600
    poll_interval = 10
    elapsed = 0
    ...
    while elapsed < timeout:
        ...
        if status == "ongoing":
            break
        if status == "completed":
            return "Competition already completed. Submission skipped."

        time.sleep(poll_interval)
        elapsed += poll_interval
    else:
        return "Competition did not start within timeout. Submission skipped."
